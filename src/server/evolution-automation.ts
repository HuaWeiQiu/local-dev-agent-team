...
export class AutomaticEvolutionController {
  ...
  start(
    ...
    const claim = (() => {
      try {
        return this.supervisor.events.claimCommand(idempotencyKey, requestHash, {
          sessionId,
          maxCycles,
          operator,
        });
    ...
    if (this.loop) {
      this.supervisor.events.releaseCommand(idempotencyKey, requestHash);
      throw new AutomaticEvolutionError("AUTOMATION_RUNNING", "Automatic evolution is already running");
    }
    let session: EvolutionAutomationSession;
    try {
      session = this.supervisor.beginAutomationSession();
    } catch (error) {
      this.supervisor.events.releaseCommand(idempotencyKey, requestHash);
      throw error;
    }
  ...
  private async runLoop(
    session: EvolutionAutomationSession,
    sessionId: string,
